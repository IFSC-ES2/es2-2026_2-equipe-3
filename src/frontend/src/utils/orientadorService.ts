...
async function garantirResposta(response: Response, statusEsperado?: number): Promise<Response> {
  if (statusEsperado !== undefined ? response.status !== statusEsperado : !response.ok) {
...
export async function cadastrarOrientador(dados: OrientadorInput): Promise<Orientador> {
  try {
    ...
    const id = location ? Number(new URL(location, resposta.url).pathname.split('/').pop()) : NaN;
...
function normalizarErro(erro: unknown): ApiError | Error {
  if (erro instanceof ApiError) {
    if (erro.status === 409) {
      return new ApiError(erro.status, 'Este e-mail já está cadastrado no sistema', erro.detalhes);
    }
    if (erro.status === 404) {
      return new ApiError(erro.status, 'Orientador não encontrado', erro.detalhes);
    }
    return erro;
  }
  ...
  return erro instanceof Error ? erro : new Error('Erro ao processar a requisição');
}
