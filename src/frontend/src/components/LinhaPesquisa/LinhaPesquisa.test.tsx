...
describe('LinhaPesquisaInput', () => {
    ...
    test('deve renderizar o componente com o input e as linhas iniciais', () => {
        render(<LinhaPesquisaInput linhas={['Inteligência Artificial', 'Redes']} onChange={onChangeMock} />);

        expect(screen.getByPlaceholderText('Ex: Formação de Professores')).toBeInTheDocument();
    ...
    test('deve exibir mensagem de erro se a linha tiver menos de 2 caracteres', async () => {
        ...
        expect(screen.getByText('A linha de pesquisa deve ter entre 2 e 80 caracteres.')).toBeInTheDocument();
        expect(onChangeMock).not.toHaveBeenCalled();
    });

    test('deve exibir mensagem de erro se a linha já existir ignorando maiúsculas/minúsculas', async () => {
        const user = userEvent.setup();
        render(<LinhaPesquisaInput linhas={['Engenharia de Software']} onChange={onChangeMock} />);
        ...
        expect(screen.getByText('Esta linha de pesquisa já foi adicionada.')).toBeInTheDocument();
        expect(onChangeMock).not.toHaveBeenCalled();
    });
    ...
    test('deve limpar a mensagem de erro ao voltar a digitar no input', async () => {
        ...
        expect(screen.queryByText('A linha de pesquisa deve ter entre 2 e 80 caracteres.')).not.toBeInTheDocument();
    });

    test('deve chamar onChange removendo a linha ao clicar no botão de remover', async () => {
        const user = userEvent.setup();
        render(<LinhaPesquisaInput linhas={['IA', 'Redes', 'Segurança']} onChange={onChangeMock} />);
