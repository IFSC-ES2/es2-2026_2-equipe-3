...
describe('OrientadorCard', () => {
    ...
    test('deve renderizar todas as informações do orientador corretamente', () => {
        ...
        expect(screen.getByText('Doutor em Computação com foco em qualidade de software.')).toBeInTheDocument();
    ...
    test('não deve renderizar campos de departamento e biografia quando não informados', () => {
        ...
        expect(screen.queryByText('Doutor em Computação com foco em qualidade de software.')).not.toBeInTheDocument();
    });

    test('deve renderizar corretamente quando a lista de áreas de pesquisa estiver vazia', () => {
        const { container } = render(<OrientadorCard orientador={orientadorSemOpcionais} />);
