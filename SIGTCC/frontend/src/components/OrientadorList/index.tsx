  ...
  useEffect(() => {
    ...
    const carregarInicial = async () => {
      ...
      try {
        ...
        if (ativo) setOrientadores(dados.filter((orientador) => orientador.ativo));
      } catch {
        if (ativo) {
          setErro('Erro ao carregar orientadores');
          setOrientadores([]);
    ...
    <section className="catalogo">
      <div className="catalogo-heading">
        <div>
          ...
          <p>Encontre professores por área de atuação e conheça suas linhas de pesquisa.</p>
        </div>
      </div>
      <form className="filtro-form" onSubmit={filtrar}>
        <label htmlFor="filtro-area">Área de atuação</label>
        <div className="filter-controls">
          <input id="filtro-area" value={area} onChange={(evento) => setArea(evento.target.value)} placeholder="Ex.: Engenharia de Software" />
          <button className="primary-button" type="submit">Filtrar</button>
        </div>
      </form>
      {carregando && <p className="feedback loading">Carregando orientadores...</p>}
      {erro && <p className="feedback erro" role="alert">{erro}</p>}
      {!carregando && !erro && orientadores.length === 0 && <p className="empty-state">Nenhum orientador encontrado</p>}
      {!carregando && orientadores.length > 0 && <div className="cards-container">{orientadores.map((orientador) => <OrientadorCard key={orientador.id} orientador={orientador} />)}</div>}
    </section>
  );
}
