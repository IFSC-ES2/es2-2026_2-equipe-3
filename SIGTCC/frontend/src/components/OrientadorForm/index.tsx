...
import {
  ...
  atualizarOrientador,
  cadastrarOrientador,
} from "../../utils/orientadorService.ts";
...
export function OrientadorForm({
  orientador,
  onSucesso,
  onErro,
}: OrientadorFormProps) {
  const [dados, setDados] = useState<OrientadorInput>(
    orientador ?? valoresIniciais,
  );
  ...
  const validar = () => {
    ...
    else if (dados.nome.trim().length < 3 || dados.nome.length > 100)
      novosErros.nome = "O nome deve ter entre 3 e 100 caracteres";
    if (!dados.email.trim()) novosErros.email = "O e-mail é obrigatório";
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(dados.email))
      novosErros.email = "Formato de e-mail inválido";
    if (dados.departamento && dados.departamento.length > 100)
      novosErros.departamento =
        "O departamento deve ter no máximo 100 caracteres";
    if (dados.vagasDisponiveis < 0)
      novosErros.vagasDisponiveis =
        "O número de vagas deve ser maior ou igual a zero";
    if (dados.biografia && dados.biografia.length > 500)
      novosErros.biografia = "A biografia deve ter no máximo 500 caracteres";
    if (dados.linhasDePesquisa.length === 0)
      novosErros.linhasDePesquisa = "Informe ao menos uma linha de pesquisa";
    setErros(novosErros);
  ...
  const enviar = async (evento: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (
        ...
        setErros(
          Object.fromEntries(
            erro.detalhes.map((detalhe) => [detalhe.campo, detalhe.mensagem]),
          ),
        );
      }
      onErro(
        erro instanceof Error ? erro.message : "Erro ao processar formulário",
      );
    } finally {
      setCarregando(false);
    }
  ...
  const erro = (campo: string) =>
    erros[campo] && <span className="erro-texto">{erros[campo]}</span>;

  return (
    <form className="orientador-form" onSubmit={enviar} noValidate>
      <div className="form-title">
        <p className="section-kicker">
          {orientador ? "Atualização de cadastro" : "Primeiro acesso"}
        </p>
      ...
      <div className="form-group pesquisa-group">
        ...
        <LinhaPesquisaInput
          linhas={dados.linhasDePesquisa}
          onChange={(linhasDePesquisa) =>
            setDados((atual) => ({ ...atual, linhasDePesquisa }))
          }
        />
